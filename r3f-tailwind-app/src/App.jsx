...
function LayoutContent({ room, setRoomAppearance }) {
  const setTemplate = useStore((s) => s.setTemplate); const currentTemplate = useStore((s) => s.currentTemplate);
  ...
  const [tab, setTab] = useState('Room Types'); const selected = structures.find((item) => selectedIds.includes(item.id));
  ...
  return <div className="layout-content"><div className="intelli-section-title"><div><h3>Layout</h3><p>Design your space structure</p></div></div><div className="intelli-tabs layout-tabs">{['Room Types', 'Dimensions', 'Structures'].map((item) => <button key={item} className={tab === item ? 'is-active' : ''} onClick={() => setTab(item)}>{item}</button>)}</div>
    {tab === 'Room Types' && <><div className="layout-subheading"><strong>Room Templates</strong></div><div className="room-template-grid">{TEMPLATE_LIST.slice(0, 6).map((template, index) => <button key={template.id} className={`room-template-card ${currentTemplate === template.id ? 'is-selected' : ''}`} onClick={() => setTemplate(template.id)}><span className={`room-template-preview preview-${index}`}><Home size={30} strokeWidth={1.1} /></span><span>{template.name.replace('Modern ', '').replace('Scandinavian ', '')}</span></button>)}</div><div className="layout-tools"><h4>Layout Tools</h4><div className="layout-tool-grid"><button onClick={() => addStructure('partition')}><strong>+</strong><span>Add Room</span></button><button onClick={() => selected && removeStructure(selected.id)} disabled={!selected}><strong>⌁</strong><span>Remove Wall</span></button><button onClick={() => addStructure('partition')}><strong>⊞</strong><span>Split Room</span></button><button onClick={() => addStructure('opening')}><strong>▥</strong><span>Merge Room</span></button></div></div></>}
    ...
    <div className="wall-settings"><h4>Wall Settings</h4><label>Wall Thickness <strong>{Number(room.wallThickness || 0.2).toFixed(2)} m</strong><input type="range" min="0.1" max="0.4" step="0.01" value={room.wallThickness || 0.2} onChange={(event) => setRoomAppearance({ wallThickness: Number(event.target.value) })} /></label></div></div>;
}
